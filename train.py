...
for epoch in tqdm(iter_counter.training_epochs()):
    ...
    for i, data_i in enumerate(tqdm(dataloader), start=iter_counter.epoch_iter):
        ...
        if i % seg_step == 0:
            print("VOICI LETAPE DE SEG")
            input = trainer.get_latest_generated()[0].cuda()
            input = torch.unsqueeze(input, 0)
            seg_pred = model_seg(tff.crop(input, 0, 0, 188, 620))
            seg_pred = seg_pred.detach().max(dim=1)[1]
            seuil = torch.nn.Threshold(5.0, 0)
            print(seuil(seg_pred))
