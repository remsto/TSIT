...
from models.networks.normalization import FADE, SPADE, SPADE_FADE
...
class SPADE_FADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt):
        super().__init__()
        # attributes
        self.learned_shortcut = (fin != fout)
        ...
        # define normalization layers
        fade_config_str = opt.norm_G.replace('spectral', '')
        self.norm_0 = SPADE_FADE(fade_config_str, fin, fin, opt.nc_spade_label)
        self.norm_1 = SPADE_FADE(fade_config_str, fmiddle, fmiddle, opt.nc_spade_label)
        if self.learned_shortcut:
            self.norm_s = SPADE_FADE(fade_config_str, fin, fin, opt.nc_spade_label)

    # Note the resnet block with FADE also takes in |feat|,
    # the feature map as input
    def forward(self, x, feat, segmap):
        x_s = self.shortcut(x, feat, segmap)

        dx = self.conv_0(self.actvn(self.norm_0(x, feat, segmap)))
        dx = self.conv_1(self.actvn(self.norm_1(dx, feat, segmap)))

        out = x_s + dx

        return out

    def shortcut(self, x, feat, segmap):
        if self.learned_shortcut:
            x_s = self.conv_s(self.norm_s(x, feat, segmap))
        else:
            x_s = x
        return x_s
